...
function EmptyState({ message, to, actionLabel }) {
  return (
    <div className="text-center py-12 bg-white/5 rounded-xl">
      <p className="text-gray-500 mb-4">{message}</p>
      {to && (
        <Link to={to} className="text-red-400 hover:text-red-300 text-sm transition">
          {actionLabel} →
        </Link>
      )}
    </div>
    ...
    <div className="min-h-screen bg-[#0d0d0d] text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 pt-24">
        ...
        <section className="mb-12">
          <h2 className="text-xl font-semibold text-white mb-4">⚙️ Preferences</h2>
          <div className="bg-white/5 rounded-xl p-6 flex flex-wrap gap-8">
            <div>
              ...
              <div className="flex gap-2">
                {['sub', 'dub'].map(opt => (
                  <button key={opt}
                    onClick={() => dispatch(updatePrefs({ language: opt }))}
            ...
            <div>
              ...
              <button
                onClick={() => dispatch(updatePrefs({ autoplay: !prefs.autoplay }))}
        ...
        <section className="mb-12">
          ...
          {watchlist.length === 0 ? (
            <EmptyState message="Your watchlist is empty." to="/" actionLabel="Browse anime" />
          ) : (
        ...
        <section className="mb-12">
          ...
          {history.length === 0 ? (
            <EmptyState message="No watch history yet." to="/" actionLabel="Start watching" />
          ) : (
            <>
              <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {history.map(anime => (
                  <Link key={anime.mal_id} to={`/watch/${anime.mal_id}`}
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-1">Episode {anime.lastEpisode}</p>

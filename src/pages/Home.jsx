...
function AnimeRow({ title, selector, fetchAction }) {
  const dispatch = useDispatch();
  const { data, loading, error } = useSelector(selector);

  useEffect(() => {
    if (data.length === 0) dispatch(fetchAction());
  }, []);
...
export default function Home() {
  const dispatch = useDispatch();
  const { data: topAnime, loading: heroLoading } = useSelector(selectTop);
  const hero = topAnime?.[0];

  useEffect(() => {
    if (topAnime.length === 0) dispatch(fetchTopAnime());
  }, []);
